...
int ft_create_img(t_imag **img, t_data *data) /*hmm*/
{
    *img = mlx_new_image(data->mlx_ptr, 720, 720);
...
int init_data(t_data *data)
{
    ...
    data->img = (t_imag **)malloc(sizeof(t_imag *) * mapWidth);// !!! to understand later
    if (!data->img)
        return (-1);
    while (i < mapWidth)
    {
        data->img[i] = malloc(sizeof(t_imag));
    ...
    /*if (ft_create_img(data) < 0) num images = num stripes.
        return (-1);*/
